...
if __name__ == 'main':
    ...
    base = './4rn0/'
    resgen = 'resgen106.sdf'
    ori = 'ori_lig.sdf'
    #pkt = pocket_trunction('./akt1/4gv1.pdb',sdf_file='./akt1/resgen87.sdf')
    merge_lig_pkt(osp.join(base,'4rn0_pocket10.pdb'),osp.join(base,resgen),osp.join(base,'resgen.pdb'))
    merge_lig_pkt(osp.join(base,'4rn0_pocket10.pdb'),osp.join(base,ori),osp.join(base,'ori.pdb'))

    report = plip_parser(plip_analysis('pdbfile','outdir'))
    # return the pymol file for visualization
    plip_analysis_visual('pdbfile','outdir')
